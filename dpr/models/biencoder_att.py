...
def attention_pool(q_vector: T, ctx_seq: T) -> T:
    """
    attention based pooling
    :param q_vector: n1 x D
    :param ctx_seq: n2 x s x D
    :return:
    """
    scores = torch.einsum("id,jsd->ijs", q_vector, ctx_seq)
    alpha = torch.softmax(scores, dim=-1)
    ctx_vec = torch.einsum("ijs,jsd->ijd", alpha, ctx_seq)
    return ctx_vec


def attention_dot_product_scores(q_vector: T, ctx_vectors: T) -> T:
    """
    Tackle multiple inputs
    :param q_vector: n1 x D
    :param ctx_vectors: n1 x n2 x D
    :return:
    """
